...
export default class ShowcaseReel extends Component {
   ...
   constructor(props) {
      ...
      this.breakpoint = 900
      this.state = {
         isDesktop: window.innerWidth > this.breakpoint,
   ...
   componentDidMount() {
      ...
      if (this.props.autoLoop && this.state.isDesktop) {
         this.startLoop()
      }
   }
   ...
   loopReelItems = () => {
      ...
      if (isDesktop) {
         ...
         const nextSecondInView =
            inViewItems[1] + 2 <= reelItems.length - 1
               ? inViewItems[1] + 2
         ...
         if (!hovering && this.props.autoLoop) {
            this.resetLoop()
         }
         return
      }
