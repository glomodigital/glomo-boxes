...
class AliceCarousel extends React.PureComponent {
   constructor(props) {
      ...
      this._onMouseEnterAutoPlayHandler = this._onMouseEnterAutoPlayHandler.bind(
         this
      )
      this._onMouseLeaveAutoPlayHandler = this._onMouseLeaveAutoPlayHandler.bind(
         this
      )
   }
   ...
   componentWillReceiveProps(nextProps) {
      ...
      if (currentIndex !== slideToIndex && slideToIndex !== undefined) {
         ...
         slideNext
            ? this._slideToItem(currentIndex + 1)
            : slidePrev
              ? this._slideToItem(currentIndex - 1)
              : this._slideToItem(slideToIndex)
      }
      ...
      if (
         this.props.autoPlayActionDisabled !== autoPlayActionDisabled ||
   ...
   componentDidUpdate(prevProps) {
      if (
         this.props.autoPlayActionDisabled !==
            prevProps.autoPlayActionDisabled ||
   ...
   _calculateInitialProps(nextProps) {
      ...
      const itemWidth =
         this.stageComponent.getBoundingClientRect().width / items
      ...
      if (this.props.responsive) {
         ...
         if (Object.keys(responsive).length) {
            Object.keys(responsive).forEach(key => {
               if (key < width)
                  items =
                     Math.min(responsive[key].items, childrenLength) || items
            })
         }
      }
      return items
   ...
   _checkRecalculation(skip) {
      ...
      window.setTimeout(() => {
         const recalculate =
            !skip && (currentIndex < 0 || currentIndex >= slides.length)
      ...
      this.setState(
         {
            ...
            translate3d:
               -itemWidth *
               (currentIndex === 0 ? items : slides.length - 1 + items),
            style: {
               transition: 'transform 0ms ease-out',
            },
         },
   ...
   _isNotInfinite = () => {
      ...
      const inactiveNext =
         infinite === false && slides.length - items === currentIndex
   ...
   _prevButton() {
      ...
      const className = `alice-carousel__prev-btn-item${inactivePrev
         ? ' __inactive'
         : ''}`
   ...
   _nextButton() {
      ...
      const className = `alice-carousel__next-btn-item${inactiveNext
         ? ' __inactive'
         : ''}`
   ...
   _getActiveSlideIndex() {
      ...
      if (items === 1) {
         ...
         if (currentIndex === slidesLength + items) {
            ...
            inactiveNext ||
            (currentIndex < items && currentIndex !== 0)
         ) {
            return dotsLength
               ...
               if (i < dotsLength) {
                  ...
                  return (
                     <li
                        ...
                        onMouseEnter={this._onMouseEnterAutoPlayHandler}
                        onMouseLeave={this._onMouseLeaveAutoPlayHandler}
                        className={`alice-carousel__dots-item${this._getActiveSlideIndex() ===
                        i
                           ? ' __active'
                           : ''}`}
                     />
      ...
      return (
         ...
         >
            <div className="alice-carousel__play-btn-wrapper">
               <div
                  className={`alice-carousel__play-btn-item${this.state
                     .isPlaying
                     ? ' __pause'
                     : ''}`}
               />{' '}
         ...
         this._autoPlayIntervalId = window.setInterval(() => {
            if (!this._isHovered() && this._autoPlayIntervalId) {
               ...
               autoPlayDirection === 'rtl'
                  ? this._slidePrev(false)
                  : this._slideNext(false)
            }
         }, playInterval)
      }
   }
   ...
   _beforeTouchEnd() {
      ...
      let swipeIndex =
         this.swipePosition.direction === 'LEFT'
   ...
   _renderStageItems = (item, i) => {
      ...
      const className = `alice-carousel__stage-item${cloned &&
      infinite === false
         ? ' __cloned'
         : ''}`
