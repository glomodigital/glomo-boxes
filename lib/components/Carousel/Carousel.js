...
export default class Carousel extends Component {
   ...
   handleTouchMove(evt) {
      ...
      const lastTouch = this.state.ongoingTouches[
         this.state.ongoingTouches.length - 1
      ]
      this.setState({
         ...this.state,
         ongoingTouches: [
            ...this.state.ongoingTouches,
            this.copyTouch(touches[0]),
         ],
      })
   }
   handleTouchEnd() {
      this.resetAutoPlay()
      const touches = this.state.ongoingTouches.map(touch => {
         return touch.pageX
      })
      const change = touches[touches.length - 1] - touches[0]
      if (change > 0) {
         this.decrement()
      } else {
         this.increment()
